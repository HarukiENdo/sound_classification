...
import numpy as np
import os
from dataset import AudioDataset, AudioDataset_T
from utils import *
import random
...
def train(args):
    ...
    #------------Name setup --------------------------------
    output_dir = f"./experiment/{args.project_name}/{args.model}_optimizer_{args.optimizer}_loss_{args.loss}_lr{args.learning_rate}_n_mels{args.n_mels}_window_size{args.window_size}_hidden_dim{args.hidden_dim}_num_layers{args.num_layers}"
    ...
    train_dataset = AudioDataset_T(csv_path='train.csv',win_length_samples=win_length_samples,hop_length_samples=hop_length_samples,n_mels_value=args.n_mels, target_sample_rate=args.sample_rate, num_samples=NUM_SAMPLES, device=device) 
    val_dataset = AudioDataset_T(csv_path='val.csv',win_length_samples=win_length_samples,hop_length_samples=hop_length_samples,n_mels_value=args.n_mels, target_sample_rate=args.sample_rate, num_samples=NUM_SAMPLES, device=device) 
    test_dataset = AudioDataset_T(csv_path='test.csv',win_length_samples=win_length_samples,hop_length_samples=hop_length_samples,n_mels_value=args.n_mels, target_sample_rate=args.sample_rate, num_samples=NUM_SAMPLES, device=device)    
    ...
    #形状は(features(height),seq_length(width))
    spectrogram_height = mel_spectrogram.shape[0]
    spectrogram_width = mel_spectrogram.shape[1]
    output_class_number = len(CLASS_NAMES)

    # --------------Model setup---------------------------
    if args.model == "TransformerModel":
        print("TransformerModel") #self, input_dim, num_heads, hidden_dim, num_layers, output_class_number
        model = TransformerModel(input_dim=args.n_mels, num_heads=8, hidden_dim=args.hidden_dim, num_layers=args.num_layers, output_class_number=output_class_number) #please modify
        
    model.to(device) #transformerの入力形状は(seq_length(width),batch_size,features(height))
    input_size=(1,spectrogram_height,spectrogram_width)
    print(input_size)
    summary(model, input_size=input_size, verbose=1) #verbose=0:最小限のモデル情報、verbose=1:メモリ容量なども含める、verbose=2:さらに詳細
    ...
    with open(summary_file_path, 'w') as f:
        with redirect_stdout(f):
            summary(model, input_size, verbose=1)
    ...
    for i in range(epochs):
        ...
        for batch_idx, (input, target) in enumerate(tqdm(train_data_loader, desc="Training")):
            ...
            if batch_idx % args.valid_steps == 0 and batch_idx != 0: #所定のvalid_stepsごとにlogを取る
...
def main(args):
    ...
    if args.wandb:
      wandb.init(entity="username", name=f"{args.model}_optimizer_{args.optimizer}_loss_{args.loss}_lr{args.learning_rate}_n_mels{args.n_mels}_window_size{args.window_size}_hidden_dim{args.hidden_dim}_num_layers{args.num_layers}", project=args.project_name, config={"max_plot_points": 20000})
    try:
        train(args)
    finally:
        if args.wandb:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help='hidden dim for TransformerModel',)
    parser.add_argument(
        '--num_layers',
        type=int,
        default=4,
        help='num layers for TransformerModel',)
    parser.add_argument(
        '--window_size',
        type=int,
        default=30,
