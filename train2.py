...
def train(args):
    ...
    train_dataset = AudioDataset(csv_path='/',win_length_samples=win_length_samples,hop_length_samples=hop_length_samples,n_mels_value=args.n_mels, target_sample_rate=args.sample_rate, num_samples=NUM_SAMPLES, device=device) 
    val_dataset = AudioDataset(csv_path='/',win_length_samples=win_length_samples,hop_length_samples=hop_length_samples,n_mels_value=args.n_mels, target_sample_rate=args.sample_rate, num_samples=NUM_SAMPLES, device=device) 
    test_dataset = AudioDataset(csv_path='/',win_length_samples=win_length_samples,hop_length_samples=hop_length_samples,n_mels_value=args.n_mels, target_sample_rate=args.sample_rate, num_samples=NUM_SAMPLES, device=device)    
    ...
    for i in range(epochs):
        ...
        for batch_idx, (input, target) in enumerate(tqdm(train_data_loader, desc="Training")):
            ...
            # イテレーション単位でログを取る
            if args.wandb and batch_idx != 0: #1batch目は初期化した影響を反映するので除外する
                wandb.log({
...
def main(args):
    ...
    if args.wandb:
      wandb.init(entity="user_name", name=f"{args.model}_optimizer_{args.optimizer}_loss_{args.loss}_lr{args.learning_rate}_n_mels{args.n_mels}_window_size{args.window_size}", project=args.project_name, config={"max_plot_points": 20000})
    try:
        train(args)
    finally:
        if args.wandb:
