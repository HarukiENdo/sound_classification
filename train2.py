...
def main(args):
    ...
    if args.wandb:
      wandb.init(entity="e-haruki", name=f"{args.model}_optimizer_{args.optimizer}_loss_{args.loss}_lr{args.learning_rate}_n_mels{args.n_mels}_window_size{args.window_size}", project=args.project_name, config=args)
    try:
        train(args)
    finally:
        if args.wandb:
